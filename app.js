...
function requireLogin(){
  ...
  if(!role){
    if(confirm('Pehle login karein. Kya login page par chalte hain?')){ window.location.href='login.html'; return null; }
    return null;
  }
...
function guard(pageRole){
  ...
  var u=requireLogin();
  if(!u) return null;
  if(allowed.indexOf(u.role)===-1){ showMsg('Yeh page sirf '+allowed.join('/')+' ke liye hai - aap '+u.role+' hain.'); window.location.href='login.html'; return null; }
  return u;
}
function logout(){ sessionStorage.removeItem('educore_role'); sessionStorage.removeItem('educore_email'); window.location.href='login.html'; }
...
function rolePage(role){
  var p={superadmin:'super-admin.html',admin:'admin.html',teacher:'teacher.html',student:'student.html'};
  return p[role]||'login.html';
}
...
function buildSidebar(active, role){
  ...
  if(role==='superadmin'){
    links=[
      {icon:'🏠',label:'Overview',url:'super-admin.html'},
      {icon:'🛡️',label:'Admins / Sub-Admins',url:'super-admin-admins.html'},
      {icon:'🏫',label:'School Setup',url:'super-admin-setup.html'},
      {icon:'📚',label:'Subjects',url:'super-admin-subjects.html'},
      {icon:'💾',label:'Backup & Restore',url:'super-admin-backup.html'},
      {icon:'🛠️',label:'System Settings',url:'super-admin-settings.html'}
    ];
  } else if(role==='admin'){
    links=[
      {icon:'📊',label:'Dashboard',url:'admin.html'},
      {icon:'🎓',label:'Students',url:'admin-students.html'},
      {icon:'👩‍🏫',label:'Teachers',url:'admin-teachers.html'},
      {icon:'📚',label:'Subjects',url:'admin-subjects.html'},
      {icon:'🏫',label:'Classes',url:'admin-classes.html'},
      {icon:'📅',label:'Attendance',url:'admin-attendance.html'},
      {icon:'📝',label:'Exams',url:'admin-exams.html'},
      {icon:'💰',label:'Fees',url:'admin-fees.html'},
      {icon:'🔔',label:'Notifications',url:'admin-notifications.html'},
      {icon:'🛡️',label:'Audit Logs',url:'admin-audit.html'},
      {icon:'⚙️',label:'Settings',url:'admin-settings.html'}
    ];
  } else if(role==='teacher'){
    links=[
      {icon:'📊',label:'Dashboard',url:'teacher.html'},
      {icon:'📅',label:'Mark Attendance',url:'teacher-attendance.html'},
      {icon:'📚',label:'Assignments',url:'teacher-assignments.html'},
      {icon:'📝',label:'Grades',url:'teacher-grades.html'},
      {icon:'📢',label:'Notifications',url:'teacher-notifications.html'},
      {icon:'📈',label:'Performance',url:'teacher-performance.html'},
      {icon:'🔑',label:'Change Password',url:'teacher-change-password.html'}
    ];
  } else {
    links=[
      {icon:'📊',label:'Dashboard',url:'student.html'},
      {icon:'📅',label:'My Attendance',url:'student-attendance.html'},
      {icon:'📚',label:'Assignments',url:'student-assignments.html'},
      {icon:'📝',label:'Results',url:'student-results.html'},
      {icon:'📄',label:'Report Card',url:'student-report.html'},
      {icon:'💰',label:'Fee Status',url:'student-fees.html'},
      {icon:'🔔',label:'Notifications',url:'student-notifications.html'},
      {icon:'🔑',label:'Change Password',url:'student-change-password.html'}
    ];
  }
